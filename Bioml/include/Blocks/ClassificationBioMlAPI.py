...
def runClassificationBioml(block: SlurmBlock):
    ...
    data = classification.DataParser(
        "../data/esterase_features.xlsx", "../data/esterase_labels.csv", sheets="ch2_20"
    )
    experiment = classification.PycaretInterface(
        "classification",
        250,
        budget_time=20,
        best_model=3,
        output_path="classification_training",
        optimize="MCC",
    )
    ...
    c = classification.split.ClusterSpliter(
        "../data/resultsDB_clu.tsv", 5, random_state=experiment.seed
    )
    X_train, X_test = c.train_test_split(data.features)
    results, models = training.generate_training_results(
        X_train, data.label, tune=True, test_data=X_test, fold_strategy=c
    )
